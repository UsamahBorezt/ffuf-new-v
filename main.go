...
func ParseFlags(opts *ffuf.ConfigOptions) *ffuf.ConfigOptions {
	...
	flag.BoolVar(&opts.General.Dedup, "dedup", opts.General.Dedup, "Deduplicate results based on response body hash (skip duplicate responses)")
	flag.BoolVar(&opts.General.DNSCache, "dns-cache", opts.General.DNSCache, "Enable DNS result caching (default: true)")
	flag.BoolVar(&opts.General.Resume, "resume", opts.General.Resume, "Resume a previously interrupted scan")
	flag.IntVar(&opts.General.MaxRetries, "retries", opts.General.MaxRetries, "Number of retries for failed requests (with exponential backoff)")
